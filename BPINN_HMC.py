...
def G_terms(a,w,A,s,m,u):
    G0 = 4*(A-(m+s*u)**2 - A*u**2 + (-1+u**2)*(-s + 2*a*(1+s)*u*w - a**2*w**2)) + \
         (-1+u)**2*np.abs(m-s)**2 + 2*(-1+u**2)*(1+2*a*(1+u)*w)*np.abs(m+s) + \
         (1+u)**2*np.abs(m+s)**2 + 2*(-1+u**2)*np.abs(m-s)*(1+2*a*(-1+u)*w+np.abs(m+s))
...
class BPINN:
    def __init__(self, layers_f, layers_g):
        self.f_net = BNN(layers_f)
        self.g_net = BNN(layers_g)
        self.w_real = nn.Parameter(torch.randn(1))
        self.w_imag = nn.Parameter(torch.randn(1))
        self.A_real = nn.Parameter(torch.randn(1))
        self.A_imag = nn.Parameter(torch.randn(1))

    def get_variables(self):
        var_f = self.f_net.get_parameters_vector()
        var_g = self.g_net.get_parameters_vector()
        shared = torch.cat([self.w_real, self.w_imag, self.A_real, self.A_imag])
